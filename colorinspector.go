...
func parseRGB(input string) (RGB, error) {
	input = strings.TrimPrefix(input, "rgb(")
	input = strings.TrimSuffix(input, ")")

	parts := strings.Split(input, ",")
	if len(parts) != 3 {
		return RGB{}, fmt.Errorf("invalid RGB format: %s", input)
	}

	var rgb RGB
	var err error

	rgb.R, err = strconv.Atoi(strings.TrimSpace(parts[0]))
	if err != nil {
		return RGB{}, err
	}
	rgb.G, err = strconv.Atoi(strings.TrimSpace(parts[1]))
	if err != nil {
		return RGB{}, err
	}
	rgb.B, err = strconv.Atoi(strings.TrimSpace(parts[2]))
	if err != nil {
		return RGB{}, err
	}

	if rgb.R < 0 || rgb.R > 255 || rgb.G < 0 || rgb.G > 255 || rgb.B < 0 || rgb.B > 255 {
		return RGB{}, fmt.Errorf("RGB values must be between 0 and 255")
	}
...
func parseColor(color string) (RGB, error) {
	...
	if rgb, ok := htmlColors[color]; ok {
		return rgb, nil
	}

	if strings.HasPrefix(color, "#") || regexp.MustCompile(`^[0-9a-f]{3}$|^[0-9a-f]{6}$`).MatchString(color) {
		return parseHex(color)
	}

	if strings.HasPrefix(color, "rgb") || strings.Contains(color, ",") {
